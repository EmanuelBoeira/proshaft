...
class ShaftForceWindow: 
	def __init__(self, origin, controller):
		...
		self.xy = ttk.Radiobutton(self.root, text='xy', value=True, variable=self.plane_xy, command = self.Hide)
		self.xy.place(x=50, y=70)

		self.xz = ttk.Radiobutton(self.root, text='xz', value=False, variable=self.plane_xy, command = self.Hide)
