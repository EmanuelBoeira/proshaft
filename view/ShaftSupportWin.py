...
class ShaftSupportWindow:
	def __init__(self, origin, controller):
		...
		self.buttonAddSupp = ttk.Button(self.root, text='Adicionar', command = lambda: [self.controller.AddSupportToModel(float(self.x.get()), int(self.support.get())), self.controller.UpdateCanvas(), self.root.destroy()])
